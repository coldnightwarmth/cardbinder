import {createPoseBuffer,remoteHandPose} from './showroom-motion.mjs?v=1';
...
export function createShowroomMultiplayer({scene,room,camera,portal,bridge,columns,onDirty}) {
 ...
 function peer(id){if(id===network.id)return null;if(!peers.has(id)){const group=new THREE.Group();group.userData.excludeFloorReflection=true;const body=new THREE.Mesh(geometry,material);body.position.y=-.965;group.add(body);scene.add(group);peers.set(id,{group,target:new THREE.Vector3(),yaw:0,cards:new Map(),room:null,motion:createPoseBuffer()});}return peers.get(id);}
 function pose(id,p,time){const node=peer(id);if(!node)return;
  ...
  node.motion.push(p,performance.now(),time);node.room=p.room;
 }
 function layoutCard(display,index,count){const p=remoteHandPose(index,count);display.group.position.set(p.x,p.y,p.z);display.group.rotation.set(p.rx,p.ry,p.rz);}
 ...
 cards.forEach((card,index)=>{
  ...
  bridge.createSharedHandCard(card).then(display=>{
   ...
   layoutCard(display,held.findIndex(c=>c.id===card.id),held.length);p.group.add(display.group);onDirty();
 ...
 for(const p of peers.values()){const sample=p.motion.sample(now);if(!sample)continue;p.group.position.set(sample.x,sample.y,sample.z);p.group.rotation.y=sample.yaw;}
